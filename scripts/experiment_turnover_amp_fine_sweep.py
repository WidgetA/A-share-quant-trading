...
def analyze(df: pd.DataFrame) -> str:
    ...
    for subset_name, sdf in subsets.items():
        ...
        for window in VOLUME_WINDOWS:
            ...
            p(f"  基线(不过滤): EOD={base_eod:+.3f}%  Nxt={base_nxt:+.3f}%  "
            ...
            p()
            p(f"  ★ 保留组收益最高: {best_kept['threshold']:.2f}x  "
            ...
            p(f"    vs 基线: EOD {best_kept['avg_eod_kept'] - base_eod:+.3f}%  "
              f"Nxt {best_kept['avg_nxt_kept'] - base_nxt:+.3f}%")
            p(f"  ◆ 过滤质量最好: {best_filter_quality['threshold']:.2f}x  "
    ...
    for r in summary_rows:
        ...
        p(f"  {r['subset']:<10} {r['window']:>4}d {r['best_t']:>7.2f}x "
    ...
    if dec_rows:
        ...
        p(f"  均值: {np.mean(ts):.2f}x  中位数: {np.median(ts):.2f}x  "
          f"std: {np.std(ts):.2f}x")
    ...
    for window in VOLUME_WINDOWS:
        ...
        ba = np.array(boot)
        p(f"  {window}d:  mean={ba.mean():.3f}x  median={np.median(ba):.2f}x  "
