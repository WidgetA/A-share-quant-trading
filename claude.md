...
- **超时与批量 (2026-06)**: 深搜硬骨头(新股待挂牌/IPO 叫停/北交所迁号)单只要跑几分钟,`KIMI_VERIFY_TIMEOUT_SEC=600`(原 180s 把 kimi 在写出正确答案前掐断 → 假"超时"工具错误)。串行 600s/只必须装进 ② 步 6h 上限,所以流水线 ② **按预算封顶 `max_codes`**(≈ `_STEP_TIMEOUT_KIMI*0.8//600`),积压多则跨夜分批排空,绝不一夜 500 只把整步跑超时。
