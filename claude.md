...
**路径 B = Tushare 注册表(`stock_basic`)漏掉的码的兜底 (2026-06 实测确认)**: `stock_basic` L 并不收录全部在交易的码 —— 实测一批 2026 新股(如 001220/001237/301513,上市数月)**在 daily/快照里交易、却不在 `stock_basic` L**(`has_more=false`、加大 limit/翻页都没有)。这些码 `load_listing` 写不进名单 → 落到"快照有、名单无"的未验证集 → **kimi 兜底**核出"在交易 + 上市日"并写名单。所以 path-B 不是可有可无:它补 Tushare 注册表的洞,新股/迁号才进得了在册集。kimi 累计核清的分布见 `GET /api/audit/listing-info/findings`(在交易 / 迁移新代码 / 已退市 / 查不到 / 停牌退市中)。
