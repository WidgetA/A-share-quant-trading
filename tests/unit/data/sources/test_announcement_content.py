...
class TestNewsAnalyzerWithContentFetcher:
    """Tests for NewsAnalyzer integration with content fetcher."""

    @pytest.mark.asyncio
    async def test_analyzer_fetches_content(self):
        """Test that NewsAnalyzer fetches content before analysis."""
        ...
        # Create mocks
        mock_llm = MagicMock()
        mock_llm.batch_analyze = AsyncMock(
            return_value=[
                MagicMock(
                    success=True,
                    sentiment="positive",
        ...
        mock_filter.filter_stocks.return_value = ["000001"]
        ...
        mock_fetcher.fetch_content = AsyncMock(
            return_value=AnnouncementContent(
                url="http://test.com/ann.pdf",
                title="测试",
                content="详细的PDF内容：每10股派发现金5元",
                content_type="pdf",
                stock_codes=["000001"],
        ...
        # Create analyzer with content fetcher
        config = NewsAnalyzerConfig(fetch_full_content=True)
        analyzer = NewsAnalyzer(
            llm_service=mock_llm,
            stock_filter=mock_filter,
            sector_mapper=mock_mapper,
        ...
        # Verify LLM was called with enhanced content
        call_args = mock_llm.batch_analyze.call_args[0][0]
        assert "详细的PDF内容" in call_args[0][1]
