...
@pytest.mark.asyncio
async def test_v20_recomputes_independently_from_v16_runtime_for_each_trigger(monkeypatch):
    ...
    async def compute(*args, **kwargs):
        calls.append(service._scan_state)
        assert service._scan_state is not v16_state
        assert service._scan_state.realtime_client is not v16_client
        return await original(*args, **kwargs)

    monkeypatch.setattr(service, "_compute_morning_selection", compute)
    await service._run_decision_iteration_with_cutoff(RUN_AT)
    first = await _dispatch_manual_trigger(service, "independent-pre-cutoff")
    service._clock = lambda: POST_CUTOFF_AT
    second = await _dispatch_manual_trigger(service, "independent-post-cutoff")
    assert len(calls) == 3
    assert len(repository.selection_runs) == 3
    assert first["entry_event_id"] != second["entry_event_id"]
    assert first["symbols"] == second["symbols"]
    assert repository.commit_entry_calls == 1
    assert all(record.event_type == "ENTRY_DECISION" for record in repository.alerts.values())
    assert v16_state.initialized and v16_state.realtime_client is v16_client
...
@pytest.mark.asyncio
async def test_selection_task_keeps_mews_independent_for_both_trigger_sources(monkeypatch):
    from tests.unit.web.test_v20_auto_manual_exact_parity_acceptance import (
        POST_CUTOFF_AT,
        RUN_AT,
        _service_and_artifact,
    ...
    monkeypatch.setattr(service, "kick_mews_for_selection_trigger", kick)
    await asyncio.wait_for(service._run_decision_iteration_with_cutoff(RUN_AT), timeout=2)
    service._clock = lambda: POST_CUTOFF_AT
    result = await asyncio.wait_for(
        _dispatch_manual_trigger(service, "same-task-mews-independent"), timeout=2
    )
    assert result["calculation_result"] == "SUCCESS"
    assert repository.commit_entry_calls == 1 and len(repository.selection_runs) == 2
    assert RUN_AT in kicks and POST_CUTOFF_AT in kicks
...
@pytest.mark.asyncio
async def test_terminal_entry_rerun_persists_current_result_with_ordinary_formatter(monkeypatch):
    ...
    service, repository, _ = _service_and_artifact(monkeypatch)
    await service._run_decision_iteration_with_cutoff(RUN_AT)
    original = repository.outbox
    service._clock = lambda: POST_CUTOFF_AT
    result = await _dispatch_manual_trigger(service, "terminal-current-result-001")
    rerun = repository.alerts[result["entry_event_id"]]
    assert original is not None and rerun.event_type == original.event_type == "ENTRY_DECISION"
    assert rerun.event_id != original.event_id
    assert _render_entry_strategy_body(rerun.semantic) == _render_entry_strategy_body(
        original.semantic
    )
    assert rerun.payload["message"].splitlines()[0] == original.payload["message"].splitlines()[0]
