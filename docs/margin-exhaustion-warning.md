...
完整历史，凌晨3点任务有界续补；有上限的自动任务优先最近日期，再逐步向前补历史。原始事实
已经写入但指标重算中断时，下一次检查会比较两者末日并恢复重算。

### 发布时点与刷新时间

两融数据由交易所盘后统一汇总，Tushare的`margin`（汇总）和`margin_detail`（逐股）要到
**下一交易日09:10（北京时间）**才拿得到上一交易日的数据。这是数据本身的可用边界（上游的
发布节奏，不是系统故障），系统按它安排取数时点：

- **目标末日按发布时点截断**（`latest_published_trade_date`）：任何补数入口都不再用“今天减一天”
  当末日，而是取「此刻两融数据已经发布的最新交易日」——交易日D的数据在D之后的第一个交易日
  09:10发布，所以09:10之前可用的最新日是**再往前一个交易日**。周一凌晨可用的最新日是上周四
  （上周五的数据要等周一09:10），而不是上周五。
- **凌晨3点第⑦步只补历史**：3点跑时上一交易日的数据尚未发布，本就无从取起；截断后它只补更早
  的真实缺口，不会把“还没发布”记成缺口或`ingestion_status='FAILED'`，因此**也不会为此发飞书告警**
  ——每日维护汇总里第⑦步显示为成功，并附注“上游已发布至 YYYY-MM-DD”，说明这一趟的数据边界。
  只有真实缺口（某天已发布却没入库、或摄取失败）才继续告警。
- **09:15发布后刷新补最新日**：调度器排在发布时点之后（不是压着09:10跑），补当天新发布的
  那一交易日；若上游延迟，则每10分钟重试一次，最多6次（约到10:15）后停手，交给下一轮
  凌晨3点/次日刷新，绝不无限重试。
- **完成判据是“最新已发布日已入库”**，不是“调用没报错”：`audit_and_fill`返回
  `published_through`（此刻应有的最新交易日）与`latest_complete`（实际已完整入库的最新交易日），
  两者一致才算追平，重试循环据此收敛。
