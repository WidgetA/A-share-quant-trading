...
用户手动写。榜单按日期**只读调用 159 的现成选股接口**
`GET /api/trading/recommendations?date=`（X-API-Key，key 取 `SCAN_API_KEY`，缺省复用
本服务 `TRADING_API_KEY`；今天读内存、历史日服务端现算）。**159 是生产选股机，
绝不改动它、绝不向它发修改类请求**。**选股接口不可达/鉴权失败 ≠ 无数据**——
不可达时中止整批（否则会把策略票误判成手动票）。
