...
用户手动写。榜单按日期只读调用**本服务自己的选股接口**
`GET /api/trading/recommendations?date=`（localhost 自调用；X-API-Key 用本机交易
key（`get_trading_api_key()`），可用 `SCAN_API_KEY`/`V16_SCAN_BASE` 环境变量覆盖；
今天读内存、历史日服务端现算、超时给足 10 分钟）。**159 生产选股机与本功能零交互、
绝不改动**。**选股接口不可达/鉴权失败 ≠ 无数据**——不可达时中止整批
（否则会把策略票误判成手动票）。
