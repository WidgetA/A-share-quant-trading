...
|---------|------|--------|---------|
| 0.25.3 | 2026-08-13 | - | AST-001 新增**持仓价格预警**：飞书里可说“立航科技跌破68提醒我”或用 `/预警` 创建、查看、取消规则；Kimi 只调用 `manage-price-alerts` 的确定性脚本，规则持久化在独立 SQLite，不开放交易 key、不写业务库。后台复用券商持仓30秒行情缓存，只在交易时段处理新鲜快照；一次触发后飞书通知并停用，发送失败保留 pending 指数退避重试。 |
...
| 0.22.3 | 2026-07-08 | - | TRD-001 修复:线上保存校准点裸 500(浏览器只见 "Internal Server Error" 不是 JSON)。①快照表 `source` 列在**全部 SQL 里加双引号**(疑与 GreptimeDB SQL 关键字冲突;假库单测测不出真 SQL 问题——**教训:新 SQL 必须先在真实 GreptimeDB 验证**,重启时 ensure_schema 重跑带引号 ALTER 自愈);②校准端点异常**透传真实报错**(HTTPException detail=异常原文,不再裸 500);③前端遇非 JSON 响应显示原文而非 "Unexpected token"。 |
| 0.22.2 | 2026-07-08 | - | TRD-001 **手动校准基准点**(用户拍板:净值从 2026-07-02 收盘 1,002,872.83 起算,更早历史因计提利润失真、不再展示;xtquant 官方文档核实无任何历史资产/流水接口,校准只能人工)。快照表加 `source` 列(broker/manual,幂等 ALTER);新端点 `POST/DELETE /api/trading/equity-baseline` + 概览卡「手动校准」小表单——输入某日收盘总资产存为 manual 快照。曲线窗口起点 = 最早快照日(手动基准即起点),账本重建退居**填快照之间的空档**(锚点 = 最早 broker 快照往回推);重建推算值与手动校准值差 >1 元 → 页面警示(说明期间有费用未补或出入金)。删除仅限 manual 行,broker 真值不可删;当日不可校准(以实时轮询为准)。以后再计提利润:在计提日设新基准点即可,无需出入金流水登记。 |
| 0.22.1 | 2026-07-08 | - | TRD-001 **历史净值改由本地账本重建**(用户实测反馈:部署首日"本周收益率 +0.14%"把部署前的周内亏损全吞了——快照没有历史,而本地 trade_notes 有完整成交流水,不该只依赖券商接口)。新算法:以最早一笔券商快照为锚,从当前持仓出发按 trade_notes 成交流水逐日倒推持仓,用 backtest_daily 收盘价逐日算盈亏(买卖价差+佣金/过户费/印花税+股息+逆回购利息),`前一日总资产=后一日总资产−当日盈亏`,重建到 2026-04-27(账本完整起点);曲线=重建段+快照段+当日实时。**对不上就截断**:倒推出现负持仓(说明更早有漏记成交)→ 从该日截断并在页面如实标注,绝不显示错的数。周收益率补**盈亏金额**列;没有上周基数的起始周仍显示"--"。附带:QMT 逆回购聚合持仓(888880/204xxx/1318xx)命名「国债逆回购」;总览补**冻结资金**展示(逆回购占用挂 frozen_cash,不显示则"总资产 99 万、可用 373"没法看懂)。 |
...
2. **净值曲线** — 总资产按日曲线,inline SVG 绘制(无第三方图表库)。三段拼接:**本地账本重建段**(部署前历史)+ **快照段**(`account_equity_snapshot`,券商真值)+ **当日实时点**。
3. **每周收益率** — 近 12 个 ISO 周(周一为一周开始,北京日期),每周给**盈亏金额 + 收益率**:周收益 = 本周最后一笔 ÷ 上周最后一笔 − 1;没有上周基数的起始周不报数(return_pct=None,前端显示"--")——宁可不显示也不显示错的。红涨绿跌(A 股惯例,沿用页面已有 `gain-pos`/`gain-neg`)。

**手动校准基准点(0.22.2)** — xtquant 无任何历史资产/资金流水接口(官方文档核实:`query_stock_asset` 只有当前时点,委托/成交只有当日),而计提利润(出金)会让纯账本重建的更早历史整体偏移。解法:概览卡「手动校准」表单把某日**收盘总资产**存为 `source='manual'` 的快照行(`POST /api/trading/equity-baseline {date, total_asset}`;DELETE 删除,仅限 manual 行、broker 真值不可删;当日不可校准,以实时轮询为准)。**曲线窗口起点 = 最早快照日**——设了基准点历史就从那天起算,更早不画;账本重建退居**填快照空档**(基准点与最早 broker 快照之间的交易日)。重建推算值与手动校准值差 >1 元会在页面警示(提示期间费用未补/有出入金)。再次计提利润时在计提日设新基准点即可。当前生效基准:**2026-07-02 = 1,002,872.83**(用户 2026-07-08 拍板)。

**历史重建算法(0.22.1,`src/trading/equity_ledger.py`)** — 券商接口没有历史资产,但本地有完整账本,历史从本地重建:

1. **锚点**:最早一笔 **broker** 券商快照(部署首日收盘总资产);无快照时用当前实时总资产兜底。窗口起点 = 最早快照日(0.22.2 起,不再默认推到 2026-04-27)。
2. **持仓倒推**:从当前 broker 持仓出发,按 `trade_notes` 买卖流水逐交易日往回还原每天收盘持仓(`前一日持仓 = 后一日持仓 − 当日买入 + 当日卖出`)。逆回购事件与 888880/204xxx/1318xx 持仓不参与(不是股票)。
3. **逐日盈亏**:`pnl(d) = Σ[close(c,d)×pos(c,d) − close(c,d−1)×pos(c,d−1)] − 当日买入成本 + 当日卖出所得 − 费用(佣金+过户费+印花税) + 股息 + 逆回购利息`。收盘价取本地 `backtest_daily`(裸码),停牌/缺行向前补最近收盘价;`realized_pnl` 字段**不参与**(mark-to-market 已含平仓收益,再加就双算)。
4. **净值倒推**:`E(d−1) = E(d) − pnl(d)`,从锚点推回 **2026-04-27**(账本完整起点,`LEDGER_COMPLETE_SINCE`)。
5. **对不上就截断(CRITICAL)**:倒推中任一持仓变负 = 更早有漏记成交(漏卖出可检出;漏买入检不出,靠账本完整起点兜底)→ 从该日**截断**重建、endpoint 返回 warning、页面如实标注截断日,绝不静默显示错的历史。成交价缺失用当日收盘价代入并入 warning。
6. **口径限制**:银证转账不在账本里——转账日之前的重建绝对值整体平移(盈亏金额仍对,收益率基数失真);快照段与重建段的锚点差可反推转账,暂不自动处理。

**特殊持仓展示(0.22.1)**:QMT 把国债逆回购报成聚合持仓码 **888880**(张数×100元=冻结金额,market_value 恒 0、无名称),持仓明细将 888880 与 204xxx/1318xx 兜底命名「国债逆回购」;逆回购占用资金挂在账户 frozen_cash 里,总览在"可用资金"下方显示"冻结 xxx"(不显示的话"总资产 99 万、可用 373 元、市值 0"无法理解)。
...
**持仓价格预警扩展 (2026-08-13)**: 用户可直接说“某只持仓跌破/突破某价提醒我”,或用
`/预警` 创建、查看、取消。Kimi 不负责常驻:它只调用 `manage-price-alerts` 的确定性脚本核对
当前持仓并写入隔离规则库 `data/price_alerts.sqlite3`;服务端 `PriceAlertMonitor` 复用 broker
30 秒持仓缓存,仅在 A 股连续交易时段且快照新鲜时比较价格。规则一次性触发,飞书发送成功后
停止;发送失败保留待通知状态并退避重试。整个链路只通知不交易。

**定位与安全边界 (CRITICAL,硬性护栏)**: 查数、分析、写报告的**只读助手**,不是运维机器人。
任意飞书消息驱动一个能跑 Shell 的模型,存在提示注入风险,以下限制不可协商:
- **绝不给**:下单/撤单、任何交易路径、docker/部署/重启操作、写业务库。唯一写入例外是
  `manage-price-alerts` 确定性脚本管理隔离的价格预警规则库,不能扩展到其他文件/数据库
- **助手专用只读 key** (`ASSISTANT_READONLY_KEY`):只被明确圈定的只读 GET 端点接受
  (`/api/trading/holdings` + `/api/trading/equity-curve`);下单/撤单等修改类端点**绝不**
  认这把 key——kimi 拿到它也发不出任何交易指令。TRADING_API_KEY 永远不进 kimi 的
  环境/prompt/技能文本
...
- **M1 机器人骨架 + `/持仓` 技能**(先做,含技能库基建全套——`/持仓` 走 kimi 链路,技能
  ...
  可随时热启动,不依赖重启
- **Phase 2 自由文本通道**(0.24.0 已实现): @机器人 大白话提问 → 同一队列 → kimi 先匹配
  技能、无匹配则按任务书「自由问答资源指南」现场解决。资源:只读接口 + SQL 只读代理
  (`/api/trading/assistant-sql`,服务端强制 SELECT 族/拒多语句/自动 LIMIT/审计日志)+
  代码仓库 `/app` 只读 + Tushare HTTP(省配额)+ 网络搜索。自由任务开 thinking(现场
  发挥要推理),斜杠命令保持 --no-thinking。残余风险如实记录:kimi 有 Shell,理论上可
  绕过代理直连容器网络——靠任务书禁令 + 代理是唯一顺手路径引导;**交易接口是 key 硬隔离,
  不依赖提示词**。
