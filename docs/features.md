...
不填佣金/过户费/印花税/股息，只记**收益**（净到手利息，存独立列 `repo_income`，与平仓
收益 `realized_pnl` **物理隔离**——两种收益任何情况都分得清）。**硬性不变量（存储层守卫
强制）**：逆回购行 realized_pnl/费项/股息 恒 NULL；非逆回购行 repo_income 恒 NULL；类型
切换时不属于新类型的数值自动清空、绝不搬家；往错误字段塞非空值 → 400 拒收。导出每行
两列各自独立，核算按列取数天然分开。手动把某事件类型改成 逆回购 时后端自动
清掉 side。三栏页蓝色 tag，编辑表单只显示 价格(利率)/数量/收益；补作业页逆回购行的待补
判定=收益没填，费项列禁用。
...
- **默认「只看待补的」**：费用已填全的行滤掉，只铺作业。待补判定**只看必填费项**，
  ...
  佣金+印花税；**逆回购必填 = 收益**（费项列对逆回购禁用）。**过户费该券商不单列，
