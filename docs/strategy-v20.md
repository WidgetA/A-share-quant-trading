...
2. 09:39 通过一次 `rt_min_daily` 全日分钟线获取形成原始早盘路径；scheduler 不按秒或
   按分钟重复请求选股行情。市场宽度另用一次批量 `rt_min` 获取 09:39 终值；
...
每个有推荐票的 D0 都客观形成一个 rolling7 盘面观察批次，不以是否下单、是否有持仓、
账户资金或 V20 运行实例的启动时间为条件。批次收益是完整 V16 推荐列表各腿从原始
09:41 bar.open 到 D2 收盘的零成本毛价格收益等权平均，缺任一腿则保持缺口，不允许
删除缺失腿后计算。历史批次可以持久化为校验缓存，但缓存或交易账本不存在时不能把
本来可由 V16 信号与行情确定的结果降成 `UNKNOWN`。
...
V20 在每个交易日 09:10 后从 `V20_MEWS_SOURCE_URL` 拉取一次已经发布的 `mews_v2`
结果；若上一交易日结果缺失、滞后或不完整，则立即请求 MEWS 服务用其生产计算器补算，
随后重新拉取。校验来源日、信号可用日、数据状态和发布时点后写入自己的 PostgreSQL
快照；成功后当天不再访问上游。盘中选股和退出路径只读该快照，V20 不连接 MEWS 上游
的存储后端。
...
- V16 原始快照、独立 V16/行情派生的 rolling7 事实、当次实际消费的 HEALTH/gap、前置状态 hash 和
...
- 服务停机漏过交易日时，按交易日顺序补 `INPUT_INVALID` 终态，不能跳日恢复；rolling7
  仍按独立 V16 信号与行情历史计算，不因正式状态或运行账本断档而产生缺口。全新影子
  lineage 以显式空前驱开始，生产 lineage 必须从 checkpoint 开始。
...
09:40 起接口统一返回冲突。无论正式槽是否已有结果，都不会重拉分钟线、再次调用 V16、
复制或重推早盘消息。系统不会产生 09:39 复盘，也不会在盘后重新选股。

每个交易日的 V16 选股调用最多一次。选股成功形成的冻结 bundle 可以在同一次正式提交的
数据库重试中复用，但重试不得再次调用扫描器；若扫描器本身失败，当日槽位按时失败关闭。
自动 scheduler、人工触发和盘后接口共同遵守这一约束。

接口返回 HTTP 202 表示对应事件已经进入 durable outbox 并完成密封，不表示飞书已经接收。
调用方必须用响应中的 `entry_event_id` 检查 outbox 最终进入 `SENT`，
并核对目标飞书群。
如果需要让网络超时或未知结果可以去重重试，调用方应在首次请求前自行生成并携带
`Idempotency-Key`，随后始终原样重用；在相同 route、stream、lineage 和 config 下，
相同 key 返回同一正式事件且不会重新运行选股或重复创建运输事件。09:40 起任何调用都
返回冲突，不会创建复发事件。
...
7. 验证人工触发无应用层鉴权、无 header 自动生成请求 ID、可选 key 重试；验证 09:40
   前只产生一次正常选股且 `ENTER` 建立模型腿，09:40 后统一返回 409 且无任何重推，
