...
V20 不依赖或调用生产 V16 的 scanner、scorer 或模型文件。V20 在
`src/strategy/v20/` 和 `models/v20/` 下拥有一份从 V16 基线冻结的独立选股实现；冻结
时的选股、特征和排序语义与 V16 零差异，之后修改 V20 不得改动生产 V16 文件或制品。
因停牌、新股或数据缺失而排除不合格输入，属于生产输入校验，不是另建一套排序规则。
V20 在同一次扫描返回的完整
`recommended` 列表上做两件事：
...
### 2026-09-08 更正：完整计算，不设日内完成硬截止

用户明确否定 09:40/09:45 计算截断和超时作废。09:39 仅定义行情输入时点，
不是计算或投递必须在下一分钟完成的期限。当天完整计算必须继续执行，成功后
正常提交并推送完整推荐；慢取数、慢计算、慢提交或投递不得仅因跨过上述时点
变成 INPUT_INVALID、只读回顾或“不买”消息。真实的数据非法仍须明确报错。
建议绑定其交易日期，跨日期的旧消息仍不得冒充当天建议。已有终态账本保持审计，
不得在升级时自动重写。09:39 行情口径、入场参考价、D1/D2 卖出和止损条件不变。

全部时间使用 `Asia/Shanghai`，原始分钟时间按“分钟结束标签”解释。

### 3.1 每日入场

1. 09:15 起预热 V20 自有模型、股票池、历史行情和板块映射；
2. `rt_min_daily` 返回的集合竞价 `09:25/09:30` 原始行必须保留并参与开盘价、
   `early_volume` 和 `volume_937` 聚合；不得在 V20 collector、持久化或重放中丢弃；
3. 09:31 起采集连续交易分钟行情；
4. V20 入场输入严格截止到原始结束标签 `09:39`，不得用 09:38 或任意最新行回退；
5. 历史输入口径固定为 `STRICT_LAST_37_EXCHANGE_SESSIONS_V1`。交易所日历先冻结 D0
...
6. 对已经进入当天冻结 V16 股票池的代码，停牌、新股和来源缺失都按逐票不合格处理，
   不因为单票不合格直接否决全日，也不因“已知停牌/尚未上市”再降低分母。历史完整
   票数除以当天冻结的 V16 股票池总数必须 `>=80%`，低于 80% 为输入异常；
7. 一只最终可用于本次扫描的股票还必须具有连续的原始 `09:31..09:39` 路径、合法
   昨收及上述完整历史。实际进入 V20 自有 scanner 的股票数除以同一个冻结 V16
   股票池总数也必须 `>=80%`；历史、分钟或昨收缺失代码、实际扫描代码和覆盖率全部
   写入快照，低于 80% 为输入异常；
8. 上述两个 80% 门槛的固定分母都是冻结的 V16 股票池，不是“全市场列表完整”，
   也不是先删掉停牌、新股或缺数票之后的动态可用集合。市场宽度股票另用独立
   collector；宽度缺失或冲突不进入任何 V16 80% 分母，也不能在健康状态下否决合法
   V16 扫描。消息里的“完整推荐”只表示没有截断本次合法 V16 输出；
9. 决策快照至少记录 `history_profile_id`、逐票 `history_input_hashes`、37 日各自的
...
10. 当天完整计算得到的 `ENTER/BLOCK/NO_SIGNAL` 正常提交并推送，不设 09:40 或
    09:45 完成硬截止。取数、计算、提交和投递耗时不得单独使合法结果变成无效；
11. 自动与手动触发走同一完整计算路径。已有当天终态决定时保持幂等并允许核查；
    还没有决定时继续计算，真实输入故障明确告警并重试，不凭墙上时间生成“不买”。

数据库校验建议所属交易日期，并对状态修订、首次终态和 outbox 保持原子约束。
消息的日期有效性防止旧交易日建议冒充当天建议；日内迟到不再更换成过期文案。
已有历史失败决定保留审计，升级不会自动重写或补成过去的入场。
...
评价和退出阈值，不参与入场选股决定。所有原始候选都保留审计，仲裁必须先
...
- V20 outbox 对 relay 是至少一次请求，以 `route_id:event_id` 原子去重；真实飞书接受后
  严格回显事件、路由、payload hash、目的地和接受时刻，未知结果继续重试。
- relay 和旧 relay 适配器都使用消息携带的 `action_expiry_ts` 核对有效日期，
  新决定不再使用 09:40 日内有效期。已有历史消息保留其原始有效期与密封字节。
...
request body、时钟、交易日或强制参数。当天尚无终态时，它加速自动 scheduler 的同一
串行决策路径，完整计算后提交正常 `ENTRY_DECISION`；`ENTER` 照常建立全部模型腿。
交易日历、09:39 原始数据、覆盖率和 leader 约束仍有效，09:40/09:45 不决定计算有效性。

已有正式终态的核查不重写原决定，重新运行相同 canonical 计算，并在明确的核查消息中
展示完整结果和对照信息。这类核查保留非交易属性，与日内迟到后首次成功的正式决定区分。
失败必须明确报警，不得伪造成功；正常计算始终不调用订单或券商接口。
...
7. 验证无 header 自动生成请求 ID、可选 key 重试；让 09:39 取数跨过 09:40/09:45
   后完成，确认正常决定、完整名单和模型腿不被截断，并检查真实数据库和最终投递。
   已有终态的核查必须保持原状态不变，跨交易日的旧消息不能冒充当天建议；
