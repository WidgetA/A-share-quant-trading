...
- Web服务启动后会立即在后台触发一次不限制天数的幂等历史补全；如果GreptimeDB尚未连接或其他数据维护正在运行，任务每15秒等待，条件满足后自动开始。这个启动任务只处理MEWS，不会触发§4.1的行情/分钟/kimi统一流水线。
...
- 有界自动任务从近期向前建底时，在累计满550个连续交易日的计算上下文前将指标标为`PARTIAL`，不把截断样本上的临时分位当成生产信号；启动和手动完整建底不受此等待影响。
