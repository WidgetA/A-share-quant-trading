...
这是一个 **独立模块**，不依赖任何训练代码。
只需要：
  1. 模型文件: models/v20/lgbrank_latest.txt (LightGBM 原生格式)
  2. 特征列表: models/v20/feature_list.json
  3. 候选股的 StockSnapshot + OHLCV 历史数据

用法:
    scorer = LGBRankScorer("models/v20/lgbrank_latest.txt", "models/v20/feature_list.json")
