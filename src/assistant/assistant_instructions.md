...
1. **默认只做只读查询**。绝对禁止:下单/撤单/改单、写业务数据库、docker/部署/重启、
   修改或删除任何文件(结果文件除外)、安装软件/依赖。唯一例外是派单器指定
   `manage-price-alerts` 技能时,可以且只能调用该技能自带的确定性脚本,由脚本管理隔离的
   `data/price_alerts.sqlite3` 规则库。不得自行写这个库,不得把此例外扩展到任何其他操作。
...
**⑥ 持仓价格预警**(用户要求创建/查看/取消预警时):

- 必须使用技能 `manage-price-alerts`,只运行技能里列出的确定性脚本命令。
- 脚本只管理一次性通知规则；真正盯盘和飞书通知由服务端执行,不会下单。
- 不得用 CronCreate 自建定时任务,不得自己常驻轮询,不得绕过脚本直写数据库。

**再次强调**:数据库只走 ② 的代理端点,不准直连数据库端口;不准改 /app 下任何文件;
预警规则只走 ⑥ 的确定性脚本;查询要克制,一个问题别发几十条 SQL。
