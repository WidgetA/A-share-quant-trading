...
def build_kimi_config_toml(api_key: str, base_url: str = DEFAULT_BASE_URL) -> str:
    ...
    of a malformed value, not something to silently embed)."""
...
def ensure_kimi_config_from_env() -> bool:
    ...
    try:
        content = build_kimi_config_toml(api_key, base_url)
    ...
    logger.info("已生成 kimi 配置 %s (认证用静态 API key,base_url=%s)", dest, base_url)
    return True
