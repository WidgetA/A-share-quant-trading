...
目前支持 `/持仓`(账户概览+持仓明细)、`/韩股`(KOSPI/KOSDAQ双源行情,默认北京时间09:00)、
`/预警`(创建/查看/取消持仓价格预警,条件命中后飞书一次性通知)、
`/帮助`(能力列表,秒回;只 @ 不带命令也回它),
以及**自由问答**——直接用大白话问(如「600519 最近五天走势」「系统的选股逻辑是什么」),
kimi 现场查:数据库只读 SQL 代理(服务端强制 SELECT 族+LIMIT+审计)、账户只读接口、
容器内代码仓库(只读)、Tushare 实时行情、网络搜索。

- 接入方式:飞书官方长连接(WebSocket),不需要公网回调地址
- 群里能 @到机器人的人都能用(白名单功能暂屏蔽);助手只持有专用只读 key,
  交易接口物理上不可达。价格预警是唯一受控写入能力:只能由仓库内确定性脚本写
  `data/price_alerts.sqlite3`,不能下单、撤单或改业务库
...
MEWS及全部组成指标；统一“数据检查和补充”会建立和修复历史数据，09:15（上游09:10发布后）自动刷新最新数据。
当前定义、完整公式和指标解释见
